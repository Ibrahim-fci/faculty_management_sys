...
let validationError=document.querySelector('.validation');
...
const level=window.location.pathname.slice(7)  //get level from url
...
postCreateBtn.addEventListener('click',(e)=>{
    ...
    fetch('/api/createPostByDocOrStudent',{
        ...
        if(result.success == true){
            ...
            validationError.innerHTML = `<div class="alert alert-danger">${result.message}</div>`
        }
    })
})
